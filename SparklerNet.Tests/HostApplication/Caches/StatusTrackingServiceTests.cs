...
public class StatusTrackingServiceTests
{
    ...
    [Fact]
        ...
        public async Task WhenEdgeNodeComesOnlineAgain_DeviceStatusRequiresReset()
        {
            ...
            Assert.False(await _statusService.IsOnline(groupId, edgeNodeId, deviceId), "Device should be offline when EdgeNode is offline");
            
            // Set EdgeNode online again
            await _statusService.UpdateEdgeNodeOnlineStatus(groupId, edgeNodeId, true, 1, 4000);
            Assert.True(await _statusService.IsOnline(groupId, edgeNodeId, null), "EdgeNode should be online after reconnection");
